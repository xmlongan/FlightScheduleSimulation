...
def get_flights(file_path):
    ...
    flights = flights.sort_values(by=['原机号','计飞'],
                                     ascending = [True,True])
    flights = flights[flights.原机号.notnull()]
...
## arrange the flights by airplanes
# each airplane takes one row with variate lenght, i.e.,  
# its flights sequence lenght.
#
def get_flight_seq(flights):
